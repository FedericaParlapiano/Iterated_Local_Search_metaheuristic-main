...
def create_petrinet(nodes, edges):
    ...
    tran = []
    i = 0
    # per ogni nodo dell'ig aggiunge alla lista tran un oggetto di tipo Tansation(<nome>, <etichetta>)
...
def acceptance(V, W, curr_e, struct_move, align_move, fo_curr, best_e, fo_best,
    ...
    if fo > fo_curr:
        ...
        if ris:  # ris = 1 take from sim annealing
            sa_count += 1 # incrementa il numero di volte che è stata accettata una soluzione peggiorativa dal simulationg annealing
...
def simulatedannealing(fo_curr, fo1, T):
    div = division((fo_curr - fo1), T, 5)
    prob = math.exp(div)
    z = random()
    if z < prob:
        return 1, prob, z
