...
# funzione che restituisce due liste ciascuna contenente un elemento che è un dizionario chiave valore
# dove la chiave è un id incrementale e il valore è il nodo in cui arriva/parte un arco della traccia passata come argomento
def pick_aligned_trace(trace, net, initial_marking, final_marking):
    # crea un dizionario, in cui  l'elemento con chiave 'alignment' è la traccia allineata
    ...
    # per ogni arco aggiunge al dizionario temp il nodo di arrivo dell'arco
    for edge in aligned_traces['alignment']:
        id += 1
        temp.append((id, edge[1]))
    al.append(temp)

    # per ogni arco aggiunge al dizionario temp1 il nodo di partenza dell'arco
    for edge in aligned_traces['alignment']:
        id1 += 1
